...
class _DynamicGoogleMapState extends State<DynamicGoogleMap> {
  ...
  Future _onDragDone() async {
    ...
    try {
      var places =
          await geoLocator.placemarkFromCoordinates(currentLat, currentLong);
  ...
  Widget buildTop() {
    return Container(
      padding: EdgeInsets.only(
          top: ScreenHelper.getPaddingTop(context) + 10, left: 10, right: 10),
      child: Container(
                      ...
                      children: <Widget>[
                        ...
                        IconButton(
                            ...
                            onPressed: () async {
                              ...
                              var thanhDa = await _getLatLongFromAddress(
                                  'cu xa thanh da');
                              var choBaChieu =
                                  await _getLatLongFromAddress('cho ba chieu');
                              var cauKinhThanhDa = await _getLatLongFromAddress(
                                  'vong xoay hang xanh');
                              var duongBachDang =
                                  await _getLatLongFromAddress('ts24');
